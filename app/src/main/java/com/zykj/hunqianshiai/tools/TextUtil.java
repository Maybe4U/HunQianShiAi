...
public class TextUtil {
	 ...
	 * @return 验证通过返回true
	 */
	static public boolean isCode(String code) {
		Pattern p = null;
		Matcher m = null;
		boolean b = false;
		p = Pattern.compile("^[A-Z0-9]{5}$"); // 验证车牌号
		m = p.matcher(code);
		b = m.matches();
		// Pattern.compile("^((13[0-9])|(15[^4,\\D])|(18[^1^4,\\D]))\\d{8}").matcher(mobiles).matches();
		return b;
	...
	public static String all(String... val){
		String result = "";
		for (String str:val) {
			result += str;
		}
	...
	public static void setLisener(View view, View.OnClickListener listener ){
		if(view != null)
			view.setOnClickListener(listener);
	}

	public static void setVisibility(View v, boolean flag){
		if(v != null)
